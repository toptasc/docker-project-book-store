...
app.config['MYSQL_DATABASE_USER'] = 'm6a'
app.config['MYSQL_DATABASE_PASSWORD'] = 'M6A-password'
app.config['MYSQL_DATABASE_DB'] = 'm6abooks'
...
# Write a function named `init_m6abooks` which initilazes the bookstore db
# Create books table within sqlite db and populate with sample data
# Execute the code below only once.
def init_m6abooks():
    drop_table = 'DROP TABLE IF EXISTS m6abooks.books;'
    books_table = """
    CREATE TABLE m6abooks.books(
    ...
    """
    data = """
    INSERT INTO m6abooks.books (title, author, is_sold)
    VALUES
...
@app.route('/')
def home():
    return "Welcome to M6A Crew Library"
